...
{
    internal class GetCurrentSettings
    {
        public static void Settings_GetSettings(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "GetSettings"))
                    ...
                    try
                    {
                        ...
                        if (configJson.SellerKey == string.Empty)
                        {
                            ...
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_GetCurrentSettings);
