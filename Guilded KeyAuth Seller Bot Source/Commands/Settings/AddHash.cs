...
{
    internal class AddHash
    {
        public static void Settings_AddHash(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "AddHash"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        string hash = sections[1];

                        if (string.IsNullOrEmpty(hash))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !AddHash <hash>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_AddHash +
                                "&hash=" + hash);
