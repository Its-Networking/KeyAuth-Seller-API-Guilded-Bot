...
{
    internal class DeleteAllVars
    {
        public static void Vars_DeleteAllVars(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "DeleteAllVars"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                            "&type=" + configJson.Type_DeleteAllVars);
