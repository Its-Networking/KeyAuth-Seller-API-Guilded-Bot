...
{
    internal class CreateNewVar
    {
        public static void Vars_CreateNewVar(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "CreateNewVar"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(data) || string.IsNullOrEmpty(authed))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !CreateNewVar <name> <data> <authed 0 for no, 1 for yes>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_CreateNewVar +
                                "&name=" + name +
                                "&data=" + data +
                                "&authed=" + authed);
