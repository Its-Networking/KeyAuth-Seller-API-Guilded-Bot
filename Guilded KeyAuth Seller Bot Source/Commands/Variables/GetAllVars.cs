...
{
    internal class GetAllVars
    {
        public static void Var_FetchAllVars(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchAllVars"))
                    ...
                    try
                    {
                        ...
                        if (configJson.SellerKey == string.Empty)
                        {
                            ...
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_GetAllVars);
