...
namespace Guilded_KeyAuth_Seller_Bot.Commands.Whitelists
{
    internal class DeleteExistingWhitelist
    {
        public static void Whitelist_DeleteWhitelist(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "DeleteWhitelist"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !DeleteWhitelist <ip>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_DeleteExistingWhitelist +
                                "&ip=" + ip);
