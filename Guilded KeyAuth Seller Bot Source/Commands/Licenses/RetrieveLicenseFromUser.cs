...
{
    internal class RetrieveLicenseFromUser
    {
        public static void License_RetrieveLicenseFromUser(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "RetrieveLicenseFromuser"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !RetrieveLicenseFromUser <user>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                                "&type=" + configJson.Type_RetrieveLicenseFromUser + 
                                "&user=" + user);
