...
{
    internal class BanLicense
    {
        public static void License_BanLicense(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "BanLicense"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        string reason = sections[2];
                        string userToo = sections[3];

                        if (string.IsNullOrEmpty(key) || string.IsNullOrEmpty(reason) || string.IsNullOrEmpty(reason) || string.IsNullOrEmpty(userToo))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !BanLicense <key> <reason> <userToo 0 = no, 1 = yes>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                                "&type=" + configJson.Type_BanLicense + 
                                "&key=" + key + 
                                "&reason=" + reason + 
                                "&userToo=" + userToo);
