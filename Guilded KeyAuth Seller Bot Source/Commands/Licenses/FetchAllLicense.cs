...
{
    internal class FetchAllLicense
    {
        public static void License_FetchAllLicenses(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchAllLicenses"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                            "&type=" + configJson.Type_FetchAllLicenses);
