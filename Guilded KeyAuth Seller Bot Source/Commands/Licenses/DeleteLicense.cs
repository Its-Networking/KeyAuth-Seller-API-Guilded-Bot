...
{
    internal class DeleteLicense
    {
        public static void License_DeleteLicense(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "DeleteLicense"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        string userToo = sections[2];

                        if (string.IsNullOrEmpty(key) || string.IsNullOrEmpty(userToo))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !DeleteLicense <License> <userToo 0 = no, 1 = yes>>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                                "&type=" + configJson.Type_Del + 
                                "&key=" + key + 
                                "&userToo=" + userToo);
