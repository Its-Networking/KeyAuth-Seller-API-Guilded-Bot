...
{
    internal class FetchAllSub
    {
        public static void Sub_FetchAllSubs(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchAllSubs"))
                    ...
                    try
                    {
                        ...
                        if (configJson.SellerKey == string.Empty)
                        {
                            ...
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_FetchAllSubscriptions);
