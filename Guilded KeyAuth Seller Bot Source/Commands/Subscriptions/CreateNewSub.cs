...
{
    internal class CreateNewSub
    {
        public static void Sub_CreateNewSub(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "CreateNewSub"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !CreateNewSub <name> <level>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_CreateNewSubscription +
                                "&name=" + name +
                                "&level=" + level);
