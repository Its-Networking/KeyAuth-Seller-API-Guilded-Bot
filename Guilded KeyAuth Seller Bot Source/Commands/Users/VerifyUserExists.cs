...
{
    internal class VerifyUserExists
    {
        public static void Users_VerifyUserExists(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "VerifyUserExists"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !VerifyUserExists <user>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_VerifyUserExists + 
                                "&user=" + user);
                            AnsiConsole.WriteLine(configJson.SellerAPILink + configJson.SellerKey + configJson.Type_VerifyUserExists + user);
