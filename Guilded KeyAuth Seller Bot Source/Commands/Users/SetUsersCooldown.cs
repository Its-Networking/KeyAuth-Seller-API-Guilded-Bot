...
{
    internal class SetUsersCooldown
    {
        public static void Users_SetUsersCooldown(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "SetUsersCooldown"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(cooldown))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !SetUsersCooldown <user> <cooldown in seconds>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_SetUsersCooldown +
                                "&user=" + user +
                                "&cooldown=" + cooldown);
