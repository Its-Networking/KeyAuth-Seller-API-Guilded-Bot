...
{
    internal class SubtractFromUserSub
    {
        public static void Users_SubtractFromUserSub(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "SubtractFromUserSub"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(sub) || string.IsNullOrEmpty(seconds))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !SubtractFromUserSub <user> <sub> <seconds>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_SubtractFromUsers +
                                "&user=" + user +
                                "&sub=" + sub +
                                "&seconds=" + seconds);
