...
namespace Guilded_KeyAuth_Seller_Bot.Commands.Users
{
    ...
    {
        public static void Users_CreateUser(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "CreateUser"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(sub) || string.IsNullOrEmpty(expiry) || string.IsNullOrEmpty(pass))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !CreateUser <user> <sub> <expiry> <pass>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_AddUser +
                                "&user=" + user +
                                "&sub=" + sub +
                                "&expiry=" + expiry +
                                "&pass=" + pass);
