...
{
    internal class AddHWIDToUser
    {
        public static void Users_AddHwidToUser(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "AddHwidToUser"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(hwid))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !AddHwidToUser <user> <hwid>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_AddHWIDToUser +
                                "&user=" + user+
                                "&hwid=" + hwid);
