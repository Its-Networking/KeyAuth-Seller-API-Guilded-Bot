...
{
    internal class GetUserVarData
    {
        public static void Users_FetchUserVarData(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchUserVarData"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(user) || string.IsNullOrEmpty(var))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !FetchUserVarData <user> <var>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_GetUserVariableData +
                                "&user=" + user +
                                "&var=" + var);
