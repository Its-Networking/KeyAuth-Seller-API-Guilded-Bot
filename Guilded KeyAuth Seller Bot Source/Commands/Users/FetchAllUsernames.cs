...
{
    internal class FetchAllUsernames
    {
        public static void Users_FetchAllUsernames(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchAllUsernames"))
                    ...
                    try
                    {
                        ...
                        if (configJson.SellerKey == string.Empty)
                        {
                            ...
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_FetchAllUsernames);
