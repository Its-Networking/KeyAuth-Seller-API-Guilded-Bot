...
{
    internal class FetchAllChannels
    {
        public static void Chats_FetchAllChannels(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchChatChannels"))
                    ...
                    try
                    {
                        ...
                        if (configJson.SellerKey == string.Empty)
                        {
                            ...
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_FetchAllChannels);
