...
{
    internal class CreateNewChatChannel
    {
        public static void Chats_CreateNewChatChannel(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "CreateNewChatChannel"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        if (string.IsNullOrEmpty(name) || string.IsNullOrEmpty(delay))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !CreateNewChatChannel <channelName> <delay between msgs in seconds)");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                                "&type=" + configJson.Type_CreateNewChatChannel + 
                                "&name=" + name + 
                                "&delay=" + delay);
