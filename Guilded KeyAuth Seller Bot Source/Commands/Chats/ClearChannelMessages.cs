...
{
    internal class ClearChannelMessages
    {
        public static void Chats_ClearChannelMessages(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "ClearChannelMessages"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        string channelName = sections[1];

                        if (string.IsNullOrEmpty(channelName))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !ClearChannelMessages <channelNames>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey + 
                                "&type=" + configJson.Type_ClearChannelMessages + 
                                "&name=" + channelName);
