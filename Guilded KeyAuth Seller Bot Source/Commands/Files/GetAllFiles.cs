...
{
    internal class GetAllFiles
    {
        public static void Files_GetAllFiles(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "GetAllFiles"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                            "&type=" + configJson.Type_GetAllFiles);
