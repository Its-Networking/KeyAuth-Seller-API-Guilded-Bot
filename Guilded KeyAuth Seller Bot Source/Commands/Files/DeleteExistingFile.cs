...
{
    internal class DeleteExistingFile
    {
        public static void Files_DeleteFile(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "DeleteFile"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        string fileid = sections[1];

                        if (string.IsNullOrEmpty(fileid))
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !DeleteFile <fileid>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_DeleteFile +
                                "&fileid=" + fileid);
