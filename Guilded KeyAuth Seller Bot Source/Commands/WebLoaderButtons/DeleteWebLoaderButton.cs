...
{
    internal class DeleteWebLoaderButton
    {
        public static void Webloader_DeleteWebloaderButton(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "DeleteWebloaderButton"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        {
                            await msgCreated.ReplyAsync("Invalid Usage. Usage: !DeleteWebloaderButton <value>");
                        }
                        else
                        {
                             
                            HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                                "&type=" + configJson.Type_DeleteWebLoaderButton +
                                "&value=" + value);
