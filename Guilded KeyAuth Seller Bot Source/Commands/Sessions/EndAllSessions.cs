...
{
    internal class EndAllSessions
    {
        public static void Sessions_EndAllSessions(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "EndAllSessions"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                            "&type=" + configJson.Type_EndAllSessions);
