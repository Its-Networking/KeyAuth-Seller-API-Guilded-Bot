...
{
    internal class GetAllSessions
    {
        public static void Sessions_FetchAllSessions(GuildedBotClient client, string prefix)
        {
            client.MessageCreated
                .Where(msgCreated => msgCreated.Content.StartsWith(prefix + "FetchAllSessions"))
                .Subscribe(async msgCreated =>
                {
                    try
                    {
                        ...
                        HttpWebRequest request = (HttpWebRequest)WebRequest.Create(configJson.SellerAPILink + configJson.SellerKey +
                            "&type=" + configJson.Type_GetAllSessions);
